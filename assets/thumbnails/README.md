...
썸네일은 시리즈별 공통 사진 배경 위에 로컬 Markdown 제목을 HTML/CSS 포스터 템플릿으로 얹은 뒤, Playwright CLI와 설치된 Google Chrome으로 PNG 캡처합니다.
...
- 배경 사진에는 제목 텍스트를 직접 합성하지 않습니다.
- 게시글별 배경 이미지는 따로 만들지 않고, 같은 시리즈의 게시글은 같은 배경을 공유합니다.
- `오픈소스 기여`와 `오픈소스 기여 사례집`은 서로 다른 시리즈로 보고 별도 배경을 사용합니다.
...
- `posts/open-source-casebook/`: Velog에만 있는 오픈소스 PR 포트폴리오 글 썸네일
- `posts/programmers-python/`: 프로그래머스 Python 코딩테스트 게시글 썸네일
- `posts/hana-finance-ai/`: 하나 청년 금융인재 프로젝트 게시글 썸네일
- `photo-backgrounds/series/`: 시리즈별 공통 사진 배경 원본
- `photo-backgrounds/credits.md`: 현재 선택한 사진 출처와 라이선스 메모
- `live-velog-posts/`: 로컬 Markdown 정본 없이 Velog에만 있는 글의 썸네일 생성 매니페스트

## 배경 교체 순서

1. `photo-backgrounds/series/` 아래에 시리즈별 배경을 준비합니다.
